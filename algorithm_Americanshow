# 每次生成并测试一个组合
def HireShow(candList,candTalents,talentList):
    n = len(candList)
    hire = candList[:]
    for i in range(2**n):
        Combination = []
...
def Good(Comb,candList,candTalents,AllTalents):
    for tal in AllTalents:
        cover = False
        for cand in Comb:
            candTal = candTalents[candList.index(card)]
            if tal in candTal:
                cover = True
        if not cover:
            return Flase
...
CandidateTalents = [['Flex','Code'],['Dance','Magic'],['Sing','Magic'],['Dance','Act','Code'],['Act','Code']]
